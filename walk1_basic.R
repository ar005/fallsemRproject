walk1 <- read.csv("/home/animesh/R_projects/project/final/walk1.csv")
...
m_gFx1 = mean(walk1$gFx)
m_gFy1 = mean(walk1$gFy)
m_gFz1 = mean(walk1$gFz)

"variance of acceleration"
var_ax1 = var(walk1$ax)
var_ay1=var(walk1$ay)
var_az1= var(walk1$az)

#variance of g_force
var_gFx1 = var(walk1$gFx)
var_gFy1=var(walk1$gFy)
var_gFz1= var(walk1$gFz)

#correlation of acceleration with time
ax_t1 = cor(walk1$time,walk1$ax)
ay_t1 = cor(walk1$time,walk1$ay)
az_t1 = cor(walk1$time,walk1$az)

#correlation of g-force with time
gFx_t1 = cor(walk1$time,walk1$gFx)
gFy_t1 = cor(walk1$time,walk1$gFy)
gFz_t1 = cor(walk1$time,walk1$gFz)
